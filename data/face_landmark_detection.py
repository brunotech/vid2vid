...
faces_folder_path = os.path.join(dataset_path, f'{phase}_img/')
predictor_path = os.path.join(dataset_path, 'shape_predictor_68_face_landmarks.dat')
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor(predictor_path)

img_paths = sorted(glob.glob(f'{faces_folder_path}*'))
for i in range(len(img_paths)):
    f = img_paths[i]
    print(f"Processing video: {f}")
    save_path = os.path.join(
        dataset_path, f'{phase}_keypoints', os.path.basename(f)
    )
    ...
    for img_name in sorted(glob.glob(os.path.join(f, '*.jpg'))):
        ...
        if len(dets) > 0:
            ...
            save_name = os.path.join(save_path, f'{os.path.basename(img_name)[:-4]}.txt')
